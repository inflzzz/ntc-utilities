...
let waveformData = []; let waveformFor = null; let waveformHandle = null; let waveformZoom = 1; let waveformFocus = null; let previewAudio = null; let playbackTime = null; let playbackFrame = null;
...
function renderHistory() {
  ...
  const markup = visible.length ? visible.map(({ item, index }) => `<article class="history-item"><div class="history-icon">${item.state === 'erro' ? '!' : (item.type === 'video' ? '▸' : '♫')}</div><div class="history-copy"><strong>${safeText(item.title)}</strong><span>${safeText(item.format?.toUpperCase() || '—')} · ${safeText(item.quality || 'original')} · ${safeText(item.size || '—')} · ${safeText(item.state === 'erro' ? 'falhou' : item.time)}</span></div>${item.file ? `<button class="ghost-button" data-open="${index}">Abrir</button><button class="ghost-button" data-folder="${index}">Pasta</button>` : ''}${item.operation === 'conversion' && item.source ? `<button class="ghost-button" data-repeat-conversion="${index}">Editar</button>` : ''}</article>`).join('') : '<div class="empty-state"><p>Nenhum item neste filtro.</p></div>';
  ...
  $$('[data-repeat-conversion]').forEach(button => button.onclick = async () => { const item = history[button.dataset.repeatConversion]; if (!item?.source) return; await addMediaFiles([item.source]); $$('.nav-item').forEach(nav => nav.classList.toggle('active', nav.dataset.view === 'converter')); $$('.view').forEach(view => view.classList.toggle('active', view.id === 'converterView')); });
}
...
window.ntc.onDownloadEvent(update => {
  ...
  if (update.status === 'complete') { const completed = current; completed.status = 'concluído'; completed.file = update.file; completed.size = formatBytes(update.size); addHistory({ title: completed.title, type: completed.type, format: completed.format, quality: completed.quality, size: completed.size, file: completed.file, time: 'Agora', operation: 'download' }); queue = queue.filter(item => item.downloadId !== completed.downloadId); current = null; $('#statusPill').textContent = 'CONCLUÍDO'; $('#mediaTitle').textContent = 'Download concluído'; $('#mediaMeta').textContent = `Salvo em ${completed.folder}`; $('#downloadButton').disabled = false; persistQueue(); renderQueue(); if (completed.type === 'audio') showEditAfterDownload(completed.file, completed.title); if ($('#openFolderAfter').checked) window.ntc.openFolder(completed.folder); setTimeout(startNext, 0); }
});
...
['converterQuality', 'converterOutputName', 'metadataTitle', 'metadataArtist', 'metadataAlbum', 'metadataYear', 'metadataGenre'].forEach(id => $(`#${id}`).addEventListener('input', rememberEditorChange));
